...
2. [Unit and Integration Tests](#unit-and-integration-tests)
    1. [NC Tests](#nc-tests)
        1. [nc_index.js File](#nc_indexjs-file)
        2. [nc_coretest File](#nc_coretestjs-file)
    2. [NSFS Tests](#nsfs-tests) 
        1. [NSFS Tests General Information](#general-information)
        2. [NSFS Tests Files List](#nsfs-tests-files-list)
...
* Running NC tests - 
    - Run `NC mocha tests` with root permissions - 
        ...
        * The `run-nc-tests` command runs nc_index.js in a container.
    - Run `NC jest tests` - 
        * Command: `sudo jest --testRegex=jest_tests/test_nc`. 
        * This command runs all NC related jest tests directly on the host.
...
* Running NSFS tests -  
    * Run `NSFS tests` with root permissions - 
        * Command: `make root-perm-test`. 
        * `root-perm-test` command runs the special index file - sudo_index.js in a container.
