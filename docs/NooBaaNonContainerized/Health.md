...
* NooBaa Health CLI will analyze the following aspects - 
  ...
  - `NooBaa endpoints health`
    - Ensuring that the NooBaa endpoint is operational (running).
...
- `https_port`
    - Type: Number
    - Default: 6443
    - Description: Indicates the name of the new account.
...
- `invalid_buckets`: 
  ...
  - Description: Array of invalid buckets, invalid buckets missing valid storage path or having invalid config JSON file.
...
#### 3. Account Can Not Access New_Buckets_Path
  ...
  - Resolutions:
    - Ensure that the account's UID/GID/user has sufficient read and write permissions within the new_buckets_path file system directory.


#### 4. Account Config File Is Missing
