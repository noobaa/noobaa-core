...
## Introduction
The CLI tool is a powerful command-line interface designed to efficiently manage buckets and accounts within your system.
...
- NSFS management CLI run will create both accounts, access_keys, and buckets directories if they are missing under the config directory.
...
- **[Account Status](#account-status)**: Retrieve the current status and detailed information about specific accounts.
...
- `from_file`
    - Type: String
    - Description: Path to JSON file which includes bucket properties. When using `from_file` flag the account details must only appear inside the options JSON file.
...
- **[Bucket Status](#bucket-status)**: Retrieve the current status and detailed information about specific buckets.
...
- `owner` (Required)
    - Type: String
    - Description: Specifies the name of the owner of the bucket.
...
- `from_file`
    - Type: String
    - Description: Path to a JSON file which includes bucket properties. When using `from_file` flag the bucket details must only appear inside the options JSON file.
...
- `force`
    - Type: Boolean
    - Description: Forcefully delete bucket if the bucket is not empty.
...
- `debug`
    - Type: Number
    - Description: Specifies the debug level used for increasing the log verbosity.
...
Add account while specifying the account properties in a file
```sh
