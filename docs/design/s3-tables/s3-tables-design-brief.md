...
- The swap is keyed on an **opaque version token**, regenerated on every successful
  commit.
- It is issued **in the endpoint fork serving the request**, by `table_server` over
  `table_store` - in process on a production endpoint
  ([§3.4](s3-tables-design.md#34-the-commit-mechanism)).
- **Core handles only the backing bucket - its provisioning and its deletion**, in
  `bucket_server`.
...
- The feature flag gates the listener itself: with it off there is no port, no
  `table_store` or `table_api` registration, and no operator Service or Route. A system
  that has never enabled the feature has no `table_store` collections; one that has
  keeps them.
...
*On the document:* the same checks as the IRC path - matching `table-uuid`, the assigned
`location`, write paths inside the table, the format-version cap, descent from the
current metadata, and v3 row lineage.
...
no predecessor ([§6.1.3](s3-tables-design.md#613-lifecycle-states)). The facade selects
that visibility through an SDK argument; it does not filter results itself.

**No live child under a deleted parent**, even across a crash. Every operation that
attaches a child - `create_namespace`,
...
1. **Never cache the table pointer** on the commit path. Table-bucket and namespace
   *record* lookups may be cached; every mutation acts on the ids it resolved;
   **authorization decisions are never cached**. On a production endpoint, no
   table-catalog request reads from core.
2. **Own the cross-store compensation, with no window where the guard is blind.**
   Record first, bucket second, on create; bucket first, record second, on delete. Each
   completing or destructive step is a state transition only one actor wins; a losing
   creation deletes what it provisioned and fails. A name held by any record fails
   `AlreadyExists`, with a hint to delete and retry; `DeleteTableBucket` clears a record
   in any state, and the next create for a name clears a marked bucket whose record is
   gone ([§6.4](s3-tables-design.md#64-four-rules-for-the-sdk)).
...
- **Honour `write.metadata.previous-versions-max` (default 100, never below 1)** on
  every IRC commit.
- **Nothing expires snapshots automatically.** Scheduled snapshot expiry is a documented
  operational requirement ([§8.1](s3-tables-design.md#81-structure)).
...
**Catalog reads and writes of `metadata.json` run as the table bucket's owner**, after
the SDK authorizes `s3tables:GetTableData` or `PutTableData` for the caller.
**`ListTableBuckets` is scoped by account**: an IAM user sees its root account's table
buckets, when its policy allows the action.
...
**Deferred per-table enforcement** compiles table policies into a bucket policy on the
backing bucket at policy-write time
([§9.2](s3-tables-design.md#92-how-per-table-authorization-should-be-built-deferred)).
...
- **Backing-bucket guards.** Configuration changes, deletion and rename of a backing
  bucket are refused on every path, S3 and management RPC - **its name is immutable**.
  Always on, keyed on a marker on the bucket's own record, never a catalog lookup; only
  `delete_table_bucket`'s internal path bypasses them
