...
(`src/util/postgres_client.js:851-867`); `md_store` treats `rowCount !== 1` as
...
(`src/util/postgres_client.js:864`), so the filter must always include the record id.
...
| Format-version cap | `config.S3_TABLES_MAX_FORMAT_VERSION = 3` at release - an operator can dial back to 2; it ships at 2 until row lineage lands (§8.2) | §8.2 |
...
false, no port is opened, `table_store` and `table_api` are not registered, and the
operator creates no Service or Route. On a system that has **never** enabled the feature,
that means the `table_store` collections of §5 are never created. On a system that has,
they are not dropped - registration is what the flag gates, not the data (Toggling,
below). Nothing about the feature is reachable, which is what
...
(§6.4 rule 2), and none of them leaves a live child under a deleted parent. A creation
that crashes after losing its transition can also leave a marked bucket whose record is
gone; the next create for that name clears it (§6.1.3).
...
current state, requiring `rowCount === 1`, so one side wins. Without it, a compensation
holding the same id, and passing the same identity check, could delete a bucket that the
creation had just marked ready.

**A creation that loses that transition cleans up after itself and must not report
success.** The winner may have finished before the creation's backing bucket even
existed: a delete can move the record to `aborting`, find no bucket, and remove the
record while the creation is paused before calling core - and the creation then
provisions a marked bucket for a record that is gone. So the losing creation deletes the
bucket it created, through the internal path (§10) - safe, because the marker names the
creation's own record id - and returns `TransientFailure`.

**If it crashes before that cleanup, the bucket is ownerless, and provably so.** Its
marker names a record id with no live row, and record ids are never reused, so no actor
can ever complete it. Such a bucket can only come from a creation that never reached
`ready`, so it never held table data. It blocks exactly one thing - its derived name, for
a later `CreateTableBucket` - and that is where it is cleared (§6.1.5). No timer and no
new state: the marker is the durable record of what needs cleaning.
...
- A declarative commit cannot reach an uninitialized table, because `loadTable` does
  not return it.
- **The difference is expressed as an SDK parameter, not as facade filtering.**
  `list_tables`, `get_table_info` and `load_table` each serve both protocols, so they take
  an `include_uninitialized` argument: the S3Tables facade passes `true`, the IRC facade
  `false`. A facade that filtered results itself would be making a storage decision,
  which §3.1 rules out.
...
- *Never adopts an existing record.* A name held by a record in any state fails
  ...
  (§6.1.3). When the existing record is not `ready`, the error message carries the
  recovery hint - *"a table bucket with this name is being created or deleted; if this
  persists, delete it and retry"* - so the one state a user cannot list is discoverable at
  the moment they hit it. The message text is free-form, so the response stays AWS-shaped.
- *Clears an ownerless backing bucket.* After inserting its own record - which makes it
  the only creation holding the name - a creation whose derived backing-bucket name is
  taken by a **marked** bucket whose record id has no live row deletes that bucket
  through the internal path and proceeds (§6.1.3). A name taken by an unmarked bucket
  still fails, as §3.2 requires.
- *Errors:* `InvalidRequest` on the name; `AlreadyExists`; `TransientFailure` if
  ...
  `provisioning → ready` transition to a concurrent deletion, after deleting the bucket
  it created.
...
- **It is also the recovery path for a stuck creation**, and therefore acts on a record
  ...
  transition instead of the `ready → deleting` one. **Against a creation still running,
  both orderings are correct:** if the abort transition wins, the creation fails and
  cleans up (§6.1.3) - the caller asked for the table bucket to go away; if the
  creation's `provisioning → ready` wins first, the creation succeeds, the delete's
  transition matches nothing, and the delete re-reads the record and proceeds as an
  ordinary deletion of a `ready` table bucket, namespace count included. On a record **already** `aborting` or `deleting` - left
...
- *One visible asymmetry, and it is the documented recovery procedure.* A
  ...
  the `AlreadyExists` message says so. This is the price of never letting one creation
  abort another, and it needs no timer. For support, non-`ready` records are visible
  through a **read-only view for the system owner** - name, state, owner, age - outside
  the S3Tables API, which keeps AWS's response shape and shows only `ready`.
...
2. **Own the cross-store compensation, and leave no window where the guard is
   ...
   **The two stores now point at each other**, so reconciliation is part of this rule
   ...
   which leaves exactly three recoverable states - a record in `provisioning`, `aborting`
   or `deleting`, whose bucket may or may not exist - and one detectable leftover: a
   marked bucket whose record is gone. That can only be a creation that never reached
   `ready` and crashed after losing its transition; its marker names a record id that
   will never exist again, so the next `CreateTableBucket` for that name deletes it
   (§6.1.3, §6.1.5). A marked bucket is adopted only by the record its marker names, and
   is deleted only on behalf of that record or once that record is provably gone.
   ...
   **Identity by id is necessary but not sufficient**, because two actors on one record
   ...
   Each is a conditional update requiring `rowCount === 1`. A cleanup that loses does
   nothing; a creation that loses deletes the bucket it created and reports failure
   rather than success. Compensation flips the state
...
5. **Write the new `metadata.json`** through `object_sdk.upload_object`
   ...
   **Steps 3 and 5 - and every other catalog read or write of `metadata.json`, in
   `create_table`, `load_table` and `set_table_metadata_location` - run as the table
   bucket's owner account**, after step 1 has authorized `s3tables:GetTableData` or
   `PutTableData` for the caller. One authorization decision, in the `s3tables:`
   vocabulary, as at AWS, and bounded by the owner's own rights on its own backing bucket.
   Running them as the caller instead would re-authorize the same bytes under `s3:`
   actions, so an IAM user granted `s3tables:*` alone would be refused a `createTable` it
...
| `TableBucketNotEmpty`, `NamespaceNotEmpty` | delete refused while children remain | `409` `NamespaceNotEmptyException` (`NamespaceNotEmpty` only - IRC has no table-bucket delete) | `409` `ConflictException` |
...
§12's cross-protocol test exists to catch. Honour the property, defaulting to 100, and
clamp it to at least 1 exactly as Iceberg's own writer does
(`Math.max(1, …)` in [`TableMetadata.addPreviousFile`](https://github.com/apache/iceberg/blob/main/core/src/main/java/org/apache/iceberg/TableMetadata.java)).
Without the clamp a client-set `0` would empty the log, and §6.1.4 reads an empty
`metadata-log` as the signature of a *first* commit - so every later imperative commit on
that table would fail the descent check.
...
defaults to 3 **once the row-lineage rules below exist** - it is introduced at 2 and
raised when they land, so no build can create a v3 table before it can maintain
`next-row-id`. It gates every way a table's version can rise - `create_table` with a
...
and the backing bucket admit the same principals. `ListTableBuckets` is **scoped by
account**, as at AWS, whose action "grants permission to list all table buckets in this
account" ([S3 Tables access management](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables-setting-up.html)):
a root account sees its own table buckets; an IAM user or assumed-role session sees its
root account's, provided its identity policies allow `s3tables:ListTableBuckets`; the
system owner sees all of them.
...
| <a id="test-4"></a>4 | **Crash safety** - inject a crash between the metadata write and the swap | The table still loads at the old version; the new file is merely orphaned |
| <a id="test-5"></a>5 | **Error semantics** - one unit case per row of §7.2, asserting status **and** the rendered error shape for both facades (§7.3), plus the exception Iceberg Java and PyIceberg raise for each commit row; and `listTables` on a missing namespace versus an existing empty one, which must be `404` and an empty list respectively (§6.1.2) | Unknown action → `400`, not ignored and not `501`; failed precondition → `409`; lost swap → `409`; provable no-op → `409` or `429`, never `503`; unobserved swap → `500`, and no other commit row surfaces as `CommitStateUnknownException`. Cheap, and the table most likely to rot |
| <a id="test-6"></a>6 | **SigV4 canonical path** - a signed request from a real client with an ARN in the path | The §3.6 defect, pinned. Until this passes, nothing else is reachable over the wire |
| <a id="test-7"></a>7 | **Client matrix** - PyIceberg, then Spark, then DuckDB over the IRC protocol; the `aws s3tables` CLI and AWS's Spark catalog client library over the S3Tables protocol | The preview's actual promise. Pin Python 3.12: PyArrow wheels lag and newer Pythons break the install |
...
| 3 | Records: schemas, collections and indexes for table buckets, namespaces and pointers; `table_store`; `table_api` plus `table_server` registered beside the object services and routed to the `md` domain; the backing-bucket marker and the `bucket_api` provisioning operations; `bucketspace_nb` methods (§3.4) | - | 6–8 d | 4, 6a, 6b |
...
| **The catalog record and the backing-bucket marker diverge** | §3.4 puts the table-bucket record in PostgreSQL and its backing bucket in `system_store`, each naming the other. A crash between the two writes, or a compensation that removes them in the wrong order, leaves a bucket whose owner is unclear - and the guards are what stand between that bucket and a `DeleteBucket` | Both create and delete remove the bucket before the record, so a crash leaves only a `provisioning`, `aborting` or `deleting` record, or a marked bucket whose record is provably gone, which the next create for that name clears (§6.4 rule 2); every completing or destructive step is gated on a state transition only one actor can win, so a creation and whatever tears it down cannot both act on one record, and no second creation ever adopts it (§6.1.3); the marker keeps the guards armed in every intermediate state. §12 test 12 injects a failure at each step, including the stalled-creation cases, and asserts on both stores |
