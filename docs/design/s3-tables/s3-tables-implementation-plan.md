...
| # | Story | Depends on |
|---|---|---|
| 1 | [Service skeleton behind the feature flag](#1-service-skeleton-behind-the-feature-flag) | - |
| 2 | [SigV4 canonical path for `s3tables`](#2-sigv4-canonical-path-for-s3tables) | 1, Spike A |
| 3 | [Operator wiring (noobaa-operator)](#3-operator-wiring-noobaa-operator) | 1 |
| 4 | [Backing-bucket provisioning and the cross-store lifecycle](#4-backing-bucket-provisioning-and-the-cross-store-lifecycle) | 5, 6b |
| 5 | [Backing-bucket guards](#5-backing-bucket-guards) | - |
| 6a | [`table_store`, `table_api` and `table_server`](#6a-table_store-table_api-and-table_server) | - |
| 6b | [Catalog transactions, state transitions and the swap](#6b-catalog-transactions-state-transitions-and-the-swap) | 6a |
| 7 | [Metadata engine core (v2)](#7-metadata-engine-core-v2) | - |
| 8 | [Format v3 row lineage](#8-format-v3-row-lineage) | 7 |
| 9 | [Metadata engine worker](#9-metadata-engine-worker) | 7 |
| 10 | [`s3_table_sdk`: skeleton, authorization, table buckets, namespaces](#10-s3_table_sdk-skeleton-authorization-table-buckets-namespaces) | 4, 5, 6b |
| 11 | [`s3_table_sdk`: table CRUD](#11-s3_table_sdk-table-crud) | 7, 10 |
| 12 | [Commit path (`commit_table`)](#12-commit-path-commit_table) | 6b, 8, 9, 11 |
| 13 | [IRC facade: config, namespaces, tables](#13-irc-facade-config-namespaces-tables) | 2, 11 |
| 14 | [IRC facade: `updateTable`](#14-irc-facade-updatetable) | 12, 13 |
| 15 | [S3Tables facade: router, errors, table buckets](#15-s3tables-facade-router-errors-table-buckets) | 2, 10, Spike B |
| 16 | [S3Tables facade: namespaces and tables](#16-s3tables-facade-namespaces-and-tables) | 11, 15 |
| 17 | [`UpdateTableMetadataLocation`](#17-updatetablemetadatalocation) | 12, 14, 16, Spike B |
| 18 | [Differential conformance CI](#18-differential-conformance-ci) | 14 |
| 19 | [Client matrix CI](#19-client-matrix-ci) | 14, 17 |
| 20 | [User documentation](#20-user-documentation) | 19 |

The design sections each story implements are listed in its [detailed
breakdown](#story-details), which is the authoritative list.

Story names link to the [detailed breakdown](#story-details) at the end.

### Parallel tracks

Stories 1, 5, 6a and 7 depend on nothing, so all three tracks start at once - track B
in two places:
...
| **B. Persistence and SDK** | 4, 5, 6a, 6b, 10, 11 | 5 and 6a in parallel; 6b after 6a; 4 after 5 and 6b; 10 after 4; 11 after 10 | Catalog records and the shared logic layer |
| **C. Metadata engine** | 7, 8, 9 | 7; then 8 and 9 in parallel | The pure transform and its worker |

**Tracks A and C share nothing** - no story in one is a dependency of a story in the
other - so they can run start to finish in parallel. The tracks join in two places:

- **Story 11** needs track C's engine (7) to build a new table's initial metadata.
- **Story 12**, the commit path, needs tracks B (6b, 11) and C (8, 9) - v3 row lineage
...
guards (5) come before backing-bucket provisioning (4), so no merged build can
...
flowchart LR
    ...
    subgraph B["Track B - persistence and SDK"]
        S6a["6a table_store, table_api"] --> S6b["6b Transactions and the swap"]
        S5["5 Guards"] --> S4["4 Backing buckets"]
        S6b --> S4
...
**The gate spans two repositories.** Story 3 lands in noobaa-operator, and an operator
that sets `CONFIG_JS_S3_TABLES_ENABLED` must not ship against a core build that lacks
the flag and the stories behind it. The operator change releases with, or after, the
core release that contains every story above - never before.
...
- Scope: `TABLES` service type; config keys `ENDPOINT_SSL_TABLES_PORT`,
  `TABLES_SERVICE_CERT_PATH` and `S3_TABLES_ENABLED` ([§3.7]) - the format-version cap
  belongs to story 7, so track C never waits on track A; `certs.TABLES`; TLS-configurable list; listener started only when
...
- Scope: the backing-bucket marker on the bucket record, the `bucket_api`
  ...
  combine them with story 6a's `table_api` calls ([§6.3]). Name validation - both reserved suffixes, 50-char cap, derived-name
  collision ([§3.2]). Record-first lifecycle over the record and transitions story 6b
  ...
  ([§6.1.5]), with a recovery hint in the already-exists message. A creation that loses
  `provisioning → ready` deletes the bucket it created; the next create for a name clears
  a marked bucket whose record is gone ([§6.1.3]). A read-only view of non-`ready`
  records for the system owner. Both create and delete remove the bucket before the
  record. No deletion token and no lease.
- Done when: [test 12] passes in full - failure injection at every step, both orderings
  of a delete racing a live creation, the late-provisioning pause, and the guard
  assertions story 5 makes executable - asserting on both stores and on the losing
  creation's response.
...
**6a. `table_store`, `table_api` and `table_server`**
- Scope: all three catalog collections - `table_buckets`, `table_namespaces` and
  `table_pointers` - with their unique partial indexes ([§5]), the pointer keyed
  `{table_bucket, namespace_name, name}` and carrying the current metadata's ETag
  ([§3.3]); the `table_api` schema for all three record types, so story 4 adds no API of
  its own; `table_server` registered through the helper shared by
  `md_server.register_rpc()` and `web_server`, only when the feature flag is on
  ([§3.4], [§3.7]); `table_api: 'md'` route; `bucketspace_nb` methods ([§6.3]); plain
  CRUD over the three collections.
- Done when: the store half of [test 16] and [test 17] pass.

**6b. Catalog transactions, state transitions and the swap**
- Scope: `update_table_metadata_location` with the swapped / not-swapped / not-found
  contract - `rowCount` read directly, zero rows resolved by a fresh read by id - and a
  distinct unknown-outcome error ([§6.3]); uninitialized pointers ([§6.1.3]); rename and
  delete conditional on the resolved namespace name and table name ([§6.1.2]); the
  table-bucket state transitions and the `ready`-only read rule ([§6.1.3], [§6.1.5]);
  transactions with a row lock on the parent for every operation that attaches a child,
  at both levels ([§6.1.2]).
- Done when: swap and locking unit tests pass; duplicate-key on create/rename maps to
...
- Scope: `next-row-id` maintenance, `first-row-id` validation, monotonicity
  assertion, safe-integer checks on the row-id fields, upgrade initialization, and
  raising the `S3_TABLES_MAX_FORMAT_VERSION` default from 2 to its released value of 3
  ([§8.2]).
...
- Scope: uncached pointer read → fetch bytes with `If-Match` → worker transform →
  ...
  `orphaned_metadata_writes` and `metadata_integrity_failures` counters, reported
  through the endpoint's Prometheus report ([§7.2], [§7.4]).
- Done when: [test 2] (cross-fork), [test 4], [test 5] (SDK level), [test 18], the
  integrated half of [test 19] and the commit half of [test 16] pass.
...
- Drive one `CreateTable` without metadata and one first commit through the library;
  record the exact `location` it writes into its `metadata.json` - with or without the
  trailing `/` the server assigned in `warehouseLocation` - and the exact string of the
  `metadata-log` entry on the commit after that ([§15]).
...
- The `location` and `metadata-log` spellings are recorded, and the literal comparisons
  in [§6.1.4] are confirmed to accept them, or the design is updated before story 17.
...
- Add configuration: the feature flag (default off, overridable through the existing
  `CONFIG_JS_*` environment mechanism the operator will use), TLS port 15443 and the
  certificate directory; add `TABLES` to the TLS-configurable services. The
  format-version cap is story 7's, since story 7 is the first code that reads it.
...
*Depends on: 5, 6b · Design: [§3.2], [§3.4], [§5], [§6.1.3], [§6.1.5], [§6.3], [§6.4]*

Work:
- Build the cross-store lifecycle on the `table_buckets` collection and the state
  transitions story 6b provides. The record carries name, owner, derived backing-bucket
  name, backing-bucket id (once it exists), encryption setting, lifecycle state and
  creation time.
- Orchestrate table-bucket create, read, list (per account, paginated) and delete over
  the `table_api` methods story 6a provides, together with backing-bucket provisioning.
  Story 5 is a dependency, so the guards exist before any backing bucket can be
  provisioned.
...
- Gate every completing and destructive step on a state transition that only one actor
  ...
  update requiring one matched row. A cleanup that loses does nothing; a creation that
  loses deletes the bucket it created and reports failure ([§6.1.3]). Compensation flips
  the state before it touches the bucket, never after.
- Never let a creation adopt an existing record: a name held by a record in any state
  fails already-exists, and when that record is not *ready* the message says to delete
  the table bucket and retry ([§6.1.5]).
- After inserting its record, let a creation whose derived backing-bucket name is taken
  by a marked bucket whose record id has no live row delete that bucket through the
  internal path and proceed; an unmarked bucket still fails ([§6.1.5]).
- Make `DeleteTableBucket` the recovery path for a record stuck in *provisioning*: it
  ...
  children ([§6.1.5]). If the creation wins `provisioning → ready` first, proceed as an
  ordinary deletion of a *ready* table bucket.
- Add a read-only view of non-*ready* records - name, state, owner, age - for the system
  owner, outside the S3Tables API ([§6.1.5]).
...
- A crash at any step leaves a *provisioning*, *aborting* or *deleting* record, or a
  marked bucket whose record is provably gone, which the next create for that name
  clears. Recovery from a crashed create is
...
- A second create for a name held by a *provisioning* record fails already-exists and
  leaves that record untouched, and its message carries the recovery hint;
  `DeleteTableBucket` on that name clears it. A delete racing a live creation is tested
  in both orders: the creation fails when the abort transition wins, and succeeds -
  followed by an ordinary deletion - when `provisioning → ready` wins first.
- The system owner's view lists a *provisioning* record while a creation is paused.
...
- [test 12], in full: a failure injected at each create and delete step leaves a
  recognisable state that the [§6.1.5] recovery clears; an S3 `DeleteBucket` against the
  half-created backing bucket is refused at every step; and the late-provisioning pause -
  a creation paused before calling core while a delete completes - ends with the creation
  deleting its bucket and failing, or, with a crash injected before that, with the next
  create clearing the ownerless bucket.
- Deleting a table bucket removes both the backing bucket and the record.

### 5. Backing-bucket guards

*Depends on: - · Design: [§3.2], [§3.7], [§6.4], [§9.1], [§10]*
...
- Add the marker field to the bucket schema; story 4 stamps it during provisioning.
- Keep the guards and the suffix refusal active regardless of the feature flag: the
  marker is part of the bucket record, so the guards answer even on a system where
  `table_store` has never been created.
...
- A bucket carrying the marker is protected from the moment it exists. Provisioning
  arrives in story 4, so these tests stamp the marker on a bucket record directly; story
  4's [test 12] then asserts the same through real provisioning.
...
*Depends on: - · Design: [§3.3], [§3.4], [§3.7], [§5], [§6.3]*
...
- Add unique partial indexes: table-bucket name per system, namespace name per table
  bucket, table name per namespace name. Key the pointer on
  `{table_bucket, namespace_name, name}`, so a table is found in one indexed lookup
  ([§5]).
- Add an internal API for **table-bucket, namespace and table CRUD**, the pointer swap
  and the table-bucket state transitions, served in-process in the endpoint when it runs
  its own metadata server, and by core otherwise - registered from the single place both
  paths use, and only when the feature flag is on, so the collections are not created on
  a system that has never enabled the feature.
- Expose all of it through the persistence interface. Story 4 builds the cross-store
  lifecycle on these methods and adds no API of its own; story 6b implements the
  transactional and conditional operations behind them.
- Implement plain CRUD over the three collections.

Acceptance criteria:
- [test 16], store half: the table-bucket, namespace and table CRUD suite passes with
  and without a local metadata server. The commit half is story 12.
...
*Depends on: 6a · Design: [§5], [§6.1.2], [§6.1.3], [§6.1.5], [§6.3], [§14]*
...
- Make every operation addressed by namespace and name - rename and delete - conditional
  on the pointer still carrying the resolved `namespace_name` and `name`, requiring one
  matched row, so a pointer moved by a concurrent rename is reported missing rather than
  written to ([§6.1.2]).
...
- With the cap at its interim value of 2, creating a v3 table is rejected as a bad
  request.
...
- Raise the `S3_TABLES_MAX_FORMAT_VERSION` default from story 7's interim 2 to 3. This
  story is what makes 3 valid, so nothing before it can create a v3 table ([§8.2]).
...
- [test 10] (v3 part): `added-rows` or `first-row-id` above 2^53 - and exactly 2^53 -
  ...
  `next-row-id`
  unchanged.
- An upgraded table has correct initial row-lineage state, and a table created at v3
  carries `next-row-id` from its first metadata file.
- The shipped default cap is 3, and v3 creation works with no configuration change.
...
- Run the transform in one worker thread per endpoint fork, created lazily on the
  first commit.
- Add `S3_TABLES_MEM_FRACTION` here, with the rest of the engine's configuration, and
  size everything from the fork's share of the pod: the container memory limit divided
  by the fork count, of which that fraction goes to the worker heap and in-flight commit
  buffers and the rest stays with ordinary S3 work ([§8.3]).
...
*Depends on: 4, 5, 6b · Design: [§3.5], [§6], [§6.4], [§9], [§10]*
...
- Add the table-bucket ownership check the IAM helper does not make: system owner and
  ...
  denied ([§9] ownership table). `ListTableBuckets` is scoped by
  account: a root account's own table buckets, an IAM user's root account's when its
  policy allows the action, and all of them for the system owner ([§9]).
...
- [test 14]: every caller in the [§9] ownership table gets the stated outcome for
  ...
  The system owner's `ListTableBuckets` returns every table bucket; a root account's
  returns its own; an IAM user's returns its root account's when its policy allows the
  action.
...
- Create table without a schema (an S3Tables `CreateTable` with no metadata): insert
  ...
  create of the same name reports already-exists. Express that as an
  `include_uninitialized` argument on list, get-info and load, which each facade passes -
  never as filtering inside a facade ([§6.1.3]).
...
*Depends on: 6b, 8, 9, 11 · Design: [§3.3], [§3.4], [§7.1], [§7.2], [§7.4]*
...
- Add counters for commit conflicts, orphaned metadata writes and metadata integrity
  failures, and report them through the endpoint's existing Prometheus report
  (`noobaa_endpoint_report.js`), so an operator can read the rates §14 relies on.
- Run all catalog object I/O as the table bucket's owner account, after the SDK's
  `s3tables:` authorization ([§7.1]).
...
- All three counters increase when their events occur and appear in the endpoint's
  Prometheus report.
- [test 16], commit half: the commit suite passes with and without a local metadata
  server.
...
*Depends on: 12, 14, 16, [Spike B](#spike-b-aws-catalog-client-library-arn-check) · Design: [§3.3], [§6.1.4], [§6.3], [§7.2], [§15]*
...
- Document table-bucket recovery: when `CreateTableBucket` reports already-exists but
  `GetTableBucket` reports not found, a creation or deletion is in progress or was
  interrupted - wait, then call `DeleteTableBucket` on the name and create again
  ([§6.1.5]). Note the system owner's view of such records for support.
...
- A new user goes from enabling the feature to a PyIceberg append and read using only
  the documentation.
- The recovery procedure above is documented and reproduced against a creation stopped
  midway.
...
[test 1]: s3-tables-design.md#test-1
[test 2]: s3-tables-design.md#test-2
[test 3]: s3-tables-design.md#test-3
[test 4]: s3-tables-design.md#test-4
[test 5]: s3-tables-design.md#test-5
[test 6]: s3-tables-design.md#test-6
[test 7]: s3-tables-design.md#test-7
[test 8]: s3-tables-design.md#test-8
[test 9]: s3-tables-design.md#test-9
[test 10]: s3-tables-design.md#test-10
[test 11]: s3-tables-design.md#test-11
[test 12]: s3-tables-design.md#test-12
[test 13]: s3-tables-design.md#test-13
[test 14]: s3-tables-design.md#test-14
[test 15]: s3-tables-design.md#test-15
[test 16]: s3-tables-design.md#test-16
[test 17]: s3-tables-design.md#test-17
[test 18]: s3-tables-design.md#test-18
[test 19]: s3-tables-design.md#test-19
[test 20]: s3-tables-design.md#test-20
[test 21]: s3-tables-design.md#test-21
