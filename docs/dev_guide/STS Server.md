...
**Note: STS is currently supported only in NooBaa ODF (Multicloud Object Gateway). It is not yet supported in NooBaa Containerized (NC).**
...
#### Role in NooBaa DB - Account Schema - Per 4.22
...
Notice - AWS ARN convention contains - <id-of-role-account> instead of <access-key-of-role-account>.
...
Find the following STS Kubrenetes resources at [NooBaa operator Repository](https://github.com/noobaa/noobaa-operator)
...
- [Key debug Log Patterns](#key-debug-log-patterns-to-search-for-in-noobaa-endpoint-pods-logs--)
...
```bash
kubectl edit cm -n <noobaa-namespace> noobaa-config
```
...
{
    "Credentials": {
        "AccessKeyId": "aaaaaaaaaaaaaEXAMPLE",
        "SecretAccessKey": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaEXAMPLE",
...
6. Bucket policies are evaluated against that account.

---

### Q-6: What will be the result of running `assume-role` more than once?
1. NooBaa will generate a pair of credentials for every assume-role request
