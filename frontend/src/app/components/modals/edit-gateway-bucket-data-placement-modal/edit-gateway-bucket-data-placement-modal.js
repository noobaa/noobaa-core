...
import template from './edit-gateway-bucket-data-placement-modal.html';
...
const formName = 'editGatewayBucketDataPlacement';
...
class EditGatewayBucketDataPlacementModalViewModel extends Observer {
    constructor({ bucket }) {
        super();
    ...
    onSubmit(values) {
        const { readPolicy, writePolicy } = values;
        action$.onNext(updateGatewayBucketPlacement(this.bucketName, readPolicy, writePolicy));
...
export default {
    viewModel: EditGatewayBucketDataPlacementModalViewModel,
    template: template
};
