 ...
 * after it finds one, it returns the account, otherwise it would throw an error
...
async function validate_bucket_args(data, action) {
    if (action === ACTIONS.DELETE || action === ACTIONS.STATUS) {
        ...
        const account = await verify_bucket_owner(data.bucket_owner, action);
        if (action === ACTIONS.ADD) data.owner_account = account._id;
