...
const { throw_cli_error, write_stdout_response, get_boolean_or_string_value, has_access_keys, set_debug_level,
...
async function add_account(data) {
    data._id = mongo_utils.mongoObjectId();
    await config_fs.create_account_config_file(data.name, data, true);
...
async function update_account(data) {
    ...
    if (update_name) {
        await config_fs.create_account_config_file(new_name, data, true, [cur_access_key]);
        await config_fs.delete_account_config_file(cur_name);
    } else {
        await config_fs.update_account_config_file(cur_name, data, update_access_key && data.access_keys,
            update_access_key && [cur_access_key]);
    }
