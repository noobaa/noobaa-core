...
if [ ! -f ${NOOBAASEC} ]; then
  ...
  if grep -q PLATFORM=aws /root/node_modules/noobaa-core/.env; then
    ...
    if grep -q AWS_PRODUCT_CODE=8q32hahci09vwgsx568lhrzwl /root/node_modules/noobaa-core/.env; then
  ...
  if grep -q PLATFORM=alyun /root/node_modules/noobaa-core/.env; then
  ...
  if grep -q PLATFORM=google /root/node_modules/noobaa-core/.env; then
    ...
    if  grep -q PAID=true /root/node_modules/noobaa-core/.env; then
      echo "AWS_INSTANCE_ID=${instance_id}" >> /root/node_modules/noobaa-core/.env
      curl -H "Content-Type: application/json" -X POST -d '{"properties__email__value":"'${instance_id}'@noobaa.com"}' https://hooks.zapier.com/hooks/catch/440450/52rywu/
    fi
  fi

  #azure specific platform fixes
  if grep -q PLATFORM=azure /root/node_modules/noobaa-core/.env; then
