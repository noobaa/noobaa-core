 ...
 */
async function list_policy_tags(req, res) {
    ...
    dbg.log1('IAM LIST POLICY TAGS (returns NoSuchEntity on every request)', params);
    const message_with_details = `Policy ${params.policy_arn} was not found`;
    const { code, http_code, type } = IamError.NoSuchEntity;
    throw new IamError({ code, message: message_with_details, http_code, type });
}

module.exports = {
    handler: list_policy_tags,
    body: {
