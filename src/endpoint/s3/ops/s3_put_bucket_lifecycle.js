...
function parse_filter(filter) {
    ...
    if (filter.Tag && filter.Tag.length === 1) {
        ...
        current_rule_filter.tag = { key: tag.Key[0], value: tag.Value[0]};
    }
    ...
    if (filter.And &&
        ...
        if (filter.And[0].Prefix &&
            ...
            filter_and.prefix = filter.And[0].Prefix[0];
        }
        filter_and.tags = _.map(filter.And[0].Tag, tag => ({ key: tag.Key[0], value: tag.Value[0]}));
        if (filter.And[0].ObjectSizeGreaterThan &&
            filter.And[0].ObjectSizeGreaterThan.length === 1) {
            filter_and.object_size_greater_than = parseInt(filter.And[0].ObjectSizeGreaterThan[0], 10);
        }
        if (filter.And[0].ObjectSizeLessThan &&
            filter.And[0].ObjectSizeLessThan.length === 1) {
            filter_and.object_size_less_than = parseInt(filter.And[0].ObjectSizeLessThan[0], 10);
        }
        current_rule_filter.and = filter_and;
    }
...
function parse_expiration(expiration) {
    ...
    } else if (expiration.ExpiredObjectDeleteMarker && expiration.ExpiredObjectDeleteMarker.length === 1) {
        output_expiration.expired_object_delete_marker = expiration.ExpiredObjectDeleteMarker[0] === 'true';
    }
