...
async function handle_request(req, res) {
  ...
  const options = {
    ...
    error_invalid_body:
      ...
      (op.body.type === "xml" ? S3Error.MalformedXML : S3Error.InvalidRequest),
...
async function _get_redirection_bucket(req, bucket) {
  const bucket_website_info = await req.object_sdk.read_bucket_sdk_website_info(
    bucket
  );
...
function check_headers(req) {
  ...
  if (req.method === "POST" || req.method === "PUT")
    s3_utils.parse_content_length(req);
...
async function authorize_request_policy(req) {
  ...
  const policy_info = await req.object_sdk.read_bucket_sdk_policy_info(
    req.params.bucket
  );
  ...
  if (s3_policy) {
    ...
    if (auth_token && auth_token.access_key) {
      ...
      if (
        (account.email.unwrap() === system_owner.unwrap() &&
          req.op_name.endsWith("bucket_policy")) ||
    ...
    if (
      !has_bucket_policy_permission(
        s3_policy,
        account ? account.email.unwrap() : undefined,
        method,
        arn_path
      )
    ) {
...
function has_bucket_policy_permission(policy, account, method, arn_path) {
  ...
  if (is_statements_fit(deny_statements, account, method, arn_path))
    return false;

  // look for explicit allows
  if (is_statements_fit(allow_statements, account, method, arn_path))
    return true;
...
function is_statements_fit(statements, account, method, arn_path) {
  for (const statement of statements) {
    ...
    dbg.log1(
      "bucket_policy: is_statements_fit",
      action_fit,
      principal_fit,
      resource_fit
    );
