...
const lambda_rest = require('./lambda_rest');
const LambdaController = require('./lambda_controller');

dbg.set_process_name('Lambda');

if (false && cluster.isMaster && process.env.LAMBDA_CLUSTER_DISABLED !== 'true') {
    // Fork workers
    for (let i = 0; i < numCPUs; i++) {
        console.warn('Spawning Lambda Server', i + 1);
        cluster.fork();
    }
    ...
    read_config_file()
        .then(config_params => {
            ...
            _.defaults(params, config_params, {
                port: process.env.LAMBDA_PORT || 80,
                ssl_port: process.env.LAMBDA_SSL_PORT || 443,
            });
        ...
        .then(certificate => {
            ...
            if (is_local_address) {
                dbg.log0('Initialize Lambda RPC with MDServer');
                const md_server = require('../server/md_server');
                return md_server.register_rpc();
            } else {
                dbg.log0('Initialize Lambda RPC to address', params.address);
...
function listen_http(port, server) {
    return new P((resolve, reject) => {
        ...
        server.listen(port, err => {
            if (err) {
                dbg.error('LAMBDA FAILED to listen', err);
                reject(err);
            } else {
                resolve();
            }
