...
class NamespaceFS {
    ...
    async read_object_stream(params, object_sdk, res) {
        ...
        try {
            ...
            if (params.rdma_info) {
                const size = await rdma_utils.read_file_to_rdma(params.rdma_info, file_reader, multi_buffer_pool);
            ...
            // end the response stream to complete the response
            res.end();
    ...
    async _upload_stream({ fs_context, params, target_file, object_sdk, offset }) {
        ...
        try {
            ...
            if (copy_source) {
                ...
                rdma_reply = await rdma_utils.write_file_from_rdma(params.rdma_info, file_writer, multi_buffer_pool);
            } else {
                await file_writer.write_entire_stream(params.source_stream, { signal: object_sdk.abort_controller.signal });
            }
    ...
    */
    ...
    _get_object_info(bucket, key, stat, isDir, is_latest = true) {
        ...
        const restore_status = GlacierBackend.get_restore_status(stat.xattr, new Date(), this._get_file_path({ key }));
