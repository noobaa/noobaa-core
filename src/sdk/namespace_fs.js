...
class NamespaceFS {
    ...
    async put_object_retention(params, object_sdk) {
        ...
        try {
            ...
            if (current_retention) {
                //TODO should check for GOVERNANCE mode and permissions, add check after implementing user permissions
                if (current_retention.mode === 'COMPLIANCE') {
                    throw new RpcError('UNAUTHORIZED');
                }
            }
            const fs_xattr = {};
            fs_xattr[XATTR_RETENTION_MODE] = JSON.stringify({
                mode: params.retention.mode,
                retain_until_date: params.retention.retain_until_date,
            });
            await this.set_fs_xattr_op(fs_context, file_path, fs_xattr, XATTR_RETENTION_MODE);
        } catch (err) {
    ...
    */
    ...
    _get_retention_mode_from_xattr(xattr) {
        if (_.isEmpty(xattr) || !xattr[XATTR_RETENTION_MODE]) return undefined;
        try {
            return JSON.parse(xattr[XATTR_RETENTION_MODE]);
        } catch (err) {
            dbg.error('Failed parsing retention mode from xattr, returning undefined', err, xattr[XATTR_RETENTION_MODE]);
            return undefined;
        }
    }
