...
class NamespaceFS {
    ...
    */
    ...
    async get_object_retention(params, object_sdk) {
        ...
        return {
            retention: {
                retain_until_date: info.retain_until_date_str,
                mode: info.mode
            }
        };
    }
    ...
    */
    ...
    _get_retention_mode_from_xattr(xattr) {
        ...
        res.retain_until_date_str = xattr[XATTR_RETENTION_DATE];
        res.retain_until_date = new Date(xattr[XATTR_RETENTION_DATE]);
        if (isNaN(res.retain_until_date.getTime())) {
    ...
    _lock_settings_from_fs_xattr(xattr) {
        ...
        if (xattr[XATTR_RETENTION_MODE] || xattr[XATTR_LEGAL_HOLD]) {
            const info = this._get_retention_mode_from_xattr(xattr);
            return {
                retention: info ? {retain_until_date: info.retain_until_date_str, mode: info.mode} : undefined,
