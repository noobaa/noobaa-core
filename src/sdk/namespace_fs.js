...
class NamespaceFS {
     ...
     * 1. open dir_file and src_file fd
     * 2. _unlink_file_ignore_enoent - which means it stats the to be deleted file, validate filter if exists, unlink safely
    ...
    async _delete_single_object(fs_context, file_path, params) {
        ...
        try {
            ...
            await this._unlink_file_ignore_enoent(fs_context, params, file_path, files.delete_version);
        } catch (err) {
            if (err.code !== 'ENOENT') throw err;
        } finally {
    ...
    */
    ...
    async _delete_single_object_versioned(fs_context, params) {
        ...
        for (;;) {
            ...
            try {
                file_path = await this._find_version_path(fs_context, params);
                await this._check_path_in_bucket_boundaries(fs_context, file_path);
                let version_info = await this._get_version_info(fs_context, file_path);
                if (!version_info) return;

                files = await this._open_files(fs_context, { src_path: file_path, delete_version: true });
                const source_file = files.delete_version.src_file;
                version_info = await this._get_version_info(fs_context, file_path, source_file);

                const deleted_latest = file_path === latest_version_path;
                if (deleted_latest) {
                    this._check_lifecycle_filter_before_deletion(params, version_info.stat);
                    if (this._is_mismatch_version_id(version_info.stat, version_id)) {
                        dbg.warn('NamespaceFS._delete_single_object_versioned mismatch version_id', file_path, version_id, this._get_version_id_by_xattr(version_info));
                        throw error_utils.new_error_code('MISMATCH_VERSION', 'file version does not match the version we asked for');
                    }
        ...
        for (;;) {
            try {
                ...
                if (latest_ver_info) {
                    ...
                    latest_ver_info = latest_file && await this._get_version_info(fs_context, latest_ver_path, latest_file);
                    ...
                    this._check_lifecycle_filter_before_deletion(params, latest_ver_info.stat);
                    const bucket_tmp_dir_path = this.get_bucket_tmpdir_full_path();
    ...
    /**
     * _unlink_file_ignore_enoent does the following - 
     ...
     * 3. calls unlinkfileat that inside checks that the path to be deleted has the same inode/fd of the file that should be deleted
     * @param {nb.NativeFSContext} fs_context 
     * @param {Object} params 
     * @param {String} file_path 
     * @param {{dir_file: nb.NativeFile, src_file: nb.NativeFile }} files 
     */
    async _unlink_file_ignore_enoent(fs_context, params, file_path, { dir_file, src_file }) {
        try {
            const fd = src_file.fd;
            const stat = await src_file.stat(fs_context);
    ...
    _check_lifecycle_filter_before_deletion(params, stat) {
        ...
        if (!should_delete) {
            const err = new RpcError('FILTER_MATCH_FAILED', 'file_matches_filter lifecycle - filter on file returned false');
