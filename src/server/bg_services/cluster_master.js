...
const MAX_RETRIES = 4;
let master_updates_enabled = true;
let bg_work = null;

exports.background_worker = background_worker;
exports.set_master_updates = set_master_updates;

function set_master_updates(req) {
    return P.resolve()
        .then(() => {
            if (bg_work) {
                return bg_work;
        ...
        .then(() => {
            console.log(`setting master_updates_enabled to ${req.rpc_params.enabled}`);
            master_updates_enabled = req.rpc_params.enabled;
        });
}
...
function background_worker() {
    ...
    bg_work = P.resolve()
        .then(() => {
            ...
            if (!master_updates_enabled) {
                dbg.log0('cluster_master is disabled');
                return;
            }
