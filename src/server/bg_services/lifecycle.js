...
async function cleanup() {
    ...
    const has_transition_policy = system_store.data.buckets.some(bucket =>
        !bucket.deleting &&
        bucket.lifecycle_configuration_rules?.some(rule =>
            rule.status === 'Enabled' && (rule.transitions?.length || rule.noncurrent_version_transitions?.length)
        )
    );
    if (!has_transition_policy) {
        dbg.log1('LIFECYCLE_TRANSITION_CLEANUP: skipped, no buckets with transition policy');
        return;
    }
