...
async function delete_incomplete_multiparts(req) {
    load_bucket(req);
    dbg.log1(`delete_incomplete_multiparts from ${req.bucket.name} that were uploaded before ${req.rpc_params.expiration_threshold}`);
    const { objects } = await list_uploads(req);
    await MDStore.instance().remove_incomplete_uploads(objects);
...
async function list_uploads(req) {
    ...
    let list_uploads_func = MDStore.instance().list_uploads.bind(MDStore.instance());
    load_bucket(req);
    ...
    if (!_.isUndefined(req.rpc_params.expiration_threshold)) {
        dbg.log0('list_uploads: listing uploads until date with expiration_threshold', req.rpc_params.expiration_threshold);
        list_uploads_func = MDStore.instance().list_uploads_until_date.bind(MDStore.instance());
