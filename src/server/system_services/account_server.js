...
function _get_role_name(role) {
    return role.name instanceof SensitiveString ? role.name.unwrap() : role.name;
}
...
/**
 * O(1) role lookup via synthetic email — same pattern as IAM users (accounts_by_email).
 ...
 * @param {string|nb.ID} owner_account_id
 */
function _validate_create_role_preconditions(role_name, owner_account_id) {
...
function _return_iam_role_info(iam_role, account_id) {
    const role_name = _get_role_name(iam_role);
    return {
        ...
        owner_access_key: iam_role.owner?.access_keys?.[0]?.access_key,
...
async function create_role(req) {
    ...
    _validate_create_role_preconditions(role_name, account_id);
...
async function list_roles(req) {
    ...
    members = members.sort((a, b) => _get_role_name(a).localeCompare(_get_role_name(b)))
