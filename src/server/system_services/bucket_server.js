...
function new_bucket_defaults(
  ...
  return {
    ...
    storage_stats: {
      ...
      last_update:
        Math.floor(now / config.MD_AGGREGATOR_INTERVAL) *
...
async function create_bucket(req) {
  ...
  if (req.rpc_params.tiering) {
    ...
    const chunk_config = chunk_config_utils.resolve_chunk_config(
      req.rpc_params.chunk_coder_config,
      req.account,
      req.system
    );
    if (!chunk_config._id) {
    ...
    const bucket_with_suffix =
      req.rpc_params.name.unwrap() + "#" + Date.now().toString(36);
    const mirrors = [
      {
        _id: system_store.new_system_store_id(),
  ...
  if (req.rpc_params.namespace) {
    const read_resources = _.compact(
      req.rpc_params.namespace.read_resources.map(
        ns_name =>
    ...
    const wr_obj =
      req.system.namespace_resources_by_name[
        req.rpc_params.namespace.write_resource
      ];
    const write_resource = wr_obj && wr_obj._id;
    if (
      req.rpc_params.namespace.read_resources &&
      (!read_resources.length ||
        read_resources.length !==
          req.rpc_params.namespace.read_resources.length)
    ) {
    ...
    const ordered_read_resources = [write_resource].concat(
      read_resources.filter(resource => resource !== write_resource)
    );
  ...
  Dispatcher.instance().activity({
    ...
    bucket: bucket._id,
    desc: `${bucket.name.unwrap()} was created by ${req.account &&
      req.account.email.unwrap()}`
  });

  // Grant the account a full access for the newly created bucket.
  if (
    req.account.allowed_buckets &&
    !req.account.allowed_buckets.full_permission
  ) {
...
function validate_pool_constraints({ mongo_pool, default_pool }) {
  if (config.ALLOW_BUCKET_CREATE_ON_INTERNAL !== true) {
    if (!(mongo_pool && mongo_pool._id) || !(default_pool && default_pool._id))
      throw new RpcError("SERVICE_UNAVAILABLE", "Non existing pool");
    if (String(mongo_pool._id) === String(default_pool._id))
      throw new RpcError(
        "SERVICE_UNAVAILABLE",
        "Not allowed to create new buckets on internal pool"
      );
  }
}
...
function _validate_s3_policy(policy, bucket_name) {
  const all_op_names = _.compact(
    _.flatMap(OP_NAME_TO_ACTION, action => [action.regular, action.versioned])
  );
  for (const statement of policy.statement) {
    ...
    for (const resource of statement.resource) {
      ...
      const resource_regex = RegExp(
        `^${resource_bucket_part.replace(/\?/g, ".?").replace(/\*/g, ".*")}$`
      );
      if (!resource_regex.test("arn:aws:s3:::" + bucket_name)) {
        throw new RpcError("MALFORMED_POLICY", "Policy has invalid resource", {
          detail: resource
        });
      }
    }
    for (const action of statement.action) {
      if (action !== "s3:*" && !all_op_names.includes(action)) {
        throw new RpcError("MALFORMED_POLICY", "Policy has invalid action", {
          detail: action
        });
      }
    }
...
async function read_bucket_sdk_info(req) {
  ...
  const reply = {
    ...
    bucket_info: await P.props({
      bucket,
      nodes_aggregate_pool: nodes_client
        .instance()
        .aggregate_nodes_by_pool(pool_names, system._id),
      hosts_aggregate_pool: nodes_client
        .instance()
        .aggregate_hosts_by_pool(null, system._id),
      num_of_objects: MDStore.instance().count_objects_of_bucket(bucket._id),
      func_configs: get_bucket_func_configs(req, bucket),
      unused_refresh_tiering_alloc: node_allocator.refresh_tiering_alloc(
        bucket.tiering
      )
...
async function update_bucket(req) {
  ...
  if (
    config.WORM_ENABLED &&
    conf &&
    conf.object_lock_enabled === "Enabled" &&
    req.rpc_params.versioning === "SUSPENDED"
  ) {
    throw new RpcError(
...
function get_bucket_changes(req, update_request, bucket, tiering_policy) {
  ...
  if (update_request.namespace) {
    get_bucket_changes_namespace(
      req,
      bucket,
      update_request,
      single_bucket_update
    );
  }
...
function get_bucket_changes_versioning(
  req,
  bucket,
  update_request,
  single_bucket_update,
  ...
  if (bucket.namespace) {
    throw new RpcError(
      "BAD_REQUEST",
      "Cannot set versioning on namespace buckets"
    );
  }
...
function get_bucket_changes_namespace(
  req,
  bucket,
  update_request,
  single_bucket_update
  ...
  const read_resources = _.compact(
    update_request.namespace.read_resources.map(
      ns_name =>
        req.system.namespace_resources_by_name[ns_name] &&
        req.system.namespace_resources_by_name[ns_name]._id
    )
  ...
  if (
    !read_resources.length ||
    read_resources.length !== update_request.namespace.read_resources.length
  ) {
    throw new RpcError("INVALID_READ_RESOURCES");
  }
  ...
  const wr_obj =
    req.system.namespace_resources_by_name[
      update_request.namespace.write_resource
    ];
  const write_resource = wr_obj && wr_obj._id;
  ...
  if (
    !_.includes(
      update_request.namespace.read_resources,
      update_request.namespace.write_resource
    )
  ) {
  ...
  _.set(
    single_bucket_update,
    "namespace.read_resources",
    ordered_read_resources
  );
  _.set(single_bucket_update, "namespace.write_resource", write_resource);
}

function get_bucket_changes_quota(
  req,
  bucket,
  quota,
  single_bucket_update,
  ...
  if (quota === null) {
    ...
    quota_event.desc = `Bucket quota was removed from ${bucket.name.unwrap()} by ${req.account &&
      req.account.email.unwrap()}`;
  } else {
    if (quota.size <= 0)
      throw new RpcError("BAD_REQUEST", "quota size must be positive");
    single_bucket_update.quota = quota;
    quota.value = size_utils
      .size_unit_to_bigint(quota.size, quota.unit)
      .toJSON();
    let used_percent = system_utils.get_bucket_quota_usage_percent(
      bucket,
      quota
    );
    if (used_percent >= 100) {
      changes.alerts.push({
        ...
        alert: `Bucket ${bucket.name.unwrap()} exceeded its configured quota of ${size_utils.human_size(
          quota.value
        )}, uploads to this bucket will be denied`,
        rule: Dispatcher.rules.once_daily
      });
      dbg.warn(
        `the bucket ${bucket.name} used capacity is more than the updated quota. uploads will be denied`
      );
    } else if (used_percent >= 90) {
      changes.alerts.push({
        sev: "INFO",
        sysid: system_store.data.systems[0]._id,
        alert: `Bucket ${bucket.name.unwrap()} exceeded 90% of its configured quota of ${size_utils.human_size(
          quota.value
        )}`,
        rule: Dispatcher.rules.once_daily
      });
    }
    quota_event.desc = `Quota of ${size_utils.human_size(
      quota.value
    )} was set on ${bucket.name.unwrap()} by ${req.account &&
      req.account.email.unwrap()}`;
  }
...
async function update_buckets(req) {
  ...
  for (const update_request of req.rpc_params) {
    ...
    const tiering_policy =
      update_request.tiering &&
    ...
    _.mergeWith(insert_changes, inserts, (existing_inserts, new_inserts) =>
      (existing_inserts || []).concat(new_inserts)
    );
    _.mergeWith(update_changes, updates, (existing_inserts, new_inserts) =>
      (existing_inserts || []).concat(new_inserts)
    );
...
function update_bucket_s3_access(req) {
  const bucket = find_bucket(req);
  const allowed_accounts = req.rpc_params.allowed_accounts.map(email =>
    system_store.get_account_by_email(email)
  );
  ...
  system_store.data.accounts.forEach(account => {
    if (
      ...
      (account.allowed_buckets && account.allowed_buckets.full_permission)
    )
      return;
...
function check_for_lambda_permission_issue(req, bucket, removed_accounts) {
  ...
  _.forEach(bucket.lambda_triggers, trigger =>
    func_store
      .instance()
      .read_func(req.system._id, trigger.func_name, trigger.func_version)
      .then(func => {
        const account = _.find(
          removed_accounts,
          acc => acc._id.toString() === func.exec_account.toString()
        );
        if (account) {
          Dispatcher.instance().alert(
            ...
            `Account’s ${account.email.unwrap()} ${bucket.name.unwrap()} bucket access was removed.
                    The configured lambda trigger for function ${
                      func.name
...
async function delete_bucket_and_objects(req) {
  ...
  if (bucket.namespace) {
    throw new RpcError(
      "BAD_REQUEST",
      "cannot perform delete_bucket_and_objects on namespace bucket"
    );
  }
  const now = new Date();
...
async function delete_bucket(req) {
  ...
  if (!req.rpc_params.internal_call) {
    Dispatcher.instance().activity({
      ...
      actor: req.account && req.account._id,
      bucket: bucket._id,
      desc: `${bucket.name.unwrap()} was deleted by ${req.account &&
        req.account.email.unwrap()}`
    });
  }
  const associated_buckets = tier_server
    .get_associated_buckets(tiering_policy)
  ...
  if (_.isEmpty(associated_buckets)) {
    ...
    tiers.push(
      ..._.compact(
        _.map(tiering_policy.tiers, tier_and_order => {
          const associated_tiering_policies = tier_server
            .get_associated_tiering_policies(tier_and_order.tier)
            .filter(
              policy_id => String(policy_id) !== String(tiering_policy._id)
            );
  ...
  const accounts_update = _.compact(
...
function delete_bucket_lifecycle(req) {
  ...
  return system_store
    ...
    .then(() => {
      let desc_string = [];
      desc_string.push(
        `lifecycle configuration rules were removed for bucket ${bucket.name.unwrap()} by ${req.account &&
          req.account.email.unwrap()}`
      );
...
function export_bucket_bandwidth_usage(req) {
  ...
  if (since > till) {
    ...
    throw new RpcError(
      "BAD_REQUEST",
      "since must be less than till for export time range"
    );
  }

  // generate csv file name:
  const since_str = new Date(since).toLocaleDateString().replace(/\//g, "-");
  const till_str = new Date(till).toLocaleDateString().replace(/\//g, "-");
  ...
  return fs_utils
    .file_delete(inner_path)
    .then(() => fs_utils.create_path(`${process.cwd()}/build/public`))
    .then(() =>
      usage_aggregator.get_bandwidth_report({
        bucket: bucket._id,
        since,
        till,
        time_range: "day"
    ...
    .then(entries => {
      const out_lines = entries.reduce(
        (lines, entry) => {
          lines.push(
            `${entry.date}, ${entry.read_count}, ${entry.read_bytes}, ${entry.write_count}, ${entry.write_bytes}`
          );
          return lines;
        },
    ...
    .catch(err => {
      dbg.error(
        "received error when writing to bucket usage csv file:",
        inner_path,
        err
      );
      throw err;
...
async function get_buckets_stats_by_content_type(req) {
  const stats = await BucketStatsStore.instance().get_all_buckets_stats({
    system: req.system._id
  });
...
function set_bucket_lifecycle_configuration_rules(req) {
  ...
  return system_store
    ...
    .then(() => {
      let desc_string = [];
      desc_string.push(
        `${bucket.name.unwrap()} was updated with lifecycle configuration rules by ${req.account &&
          req.account.email.unwrap()}`
      );
...
function get_cloud_buckets(req) {
  ...
  return P.fcall(function() {
    ...
    if (connection.endpoint_type === "AZURE") {
      let blob_svc = azure_storage.createBlobService(
        cloud_utils.get_azure_connection_string(connection)
      );
      let used_cloud_buckets = cloud_utils.get_used_cloud_targets(
        ["AZURE"],
        system_store.data.buckets,
        system_store.data.pools,
        system_store.data.namespace_resources
      );
      ...
      try {
        ...
        throw new RpcError(
          "BAD_REQUEST",
          "connection does not contain a key_file in json format"
        );
      }
      ...
      var s3 = new AWS.S3({
        ...
        signatureVersion: cloud_utils.get_s3_endpoint_signature_ver(
          connection.endpoint,
          connection.auth_method
        ),
        s3DisableBodySigning: cloud_utils.disable_s3_compatible_bodysigning(
          connection.endpoint
        ),
        httpOptions: {
      ...
      const used_cloud_buckets = cloud_utils.get_used_cloud_targets(
        ["AWS", "S3_COMPATIBLE", "FLASHBLADE", "IBM_COS"],
        system_store.data.buckets,
        system_store.data.pools,
        system_store.data.namespace_resources
      );
...
function delete_bucket_lambda_trigger(req) {
  ...
  const trigger = bucket.lambda_triggers.find(
    trig => trig._id.toString() === trigger_id
  );
  if (!trigger) {
    throw new RpcError(
      "NO_SUCH_TRIGGER",
      "This trigger does not exists: " + trigger_id
    );
  }
  return P.resolve()
    .then(() =>
...
function update_bucket_lambda_trigger(req) {
  ...
  const trigger = _.find(
    ...
    trig => trig._id.toString() === req.rpc_params.id
  );
  if (!trigger) {
    throw new RpcError(
      "NO_SUCH_TRIGGER",
      "This trigger does not exists: " + req.rpc_params._id
    );
  }
...
async function update_all_buckets_default_pool(req) {
  ...
  const buckets_with_internal_pool = _.filter(
    req.system.buckets_by_name,
    bucket => is_using_internal_storage(bucket, internal_pool)
  );
  ...
  dbg.log0(
    `Updating ${buckets_with_internal_pool.length} buckets to use ${pool_name} as default resource`
  );
  await system_store.make_changes({
    update: {
      tiers: updates
...
function validate_bucket_creation(req) {
  if (
    req.rpc_params.name.unwrap().length < 3 ||
  ...
  const bucket =
    req.system.buckets_by_name &&
    req.system.buckets_by_name[req.rpc_params.name.unwrap()];
...
function validate_trigger_update(bucket, validated_trigger) {
  dbg.log0(
    "validate_trigger_update: Chekcing new trigger is legal:",
    validated_trigger
  );
  let validate_function = true;
  _.forEach(bucket.lambda_triggers, trigger => {
    const is_same_trigger =
      String(trigger._id) === String(validated_trigger._id);
    const is_same_func =
    ...
    if (
      ...
      throw new RpcError(
        "TRIGGER_DUPLICATE",
        "This trigger is the same as an existing one"
      );
    }
  });
  ...
  return func_store
    ...
    .read_func(
      bucket.system._id,
      validated_trigger.func_name,
      validated_trigger.func_version
    )
    .then(func => {
...
function _inject_usage_to_cloud_bucket(target_name, endpoint, usage_list) {
  ...
  let using_target = usage_list.find(
    candidate_target =>
...
function find_bucket(req, bucket_name = req.rpc_params.name) {
  var bucket =
    req.system.buckets_by_name &&
    req.system.buckets_by_name[bucket_name.unwrap()];
  if (!bucket) {
...
function get_bucket_info({
  ...
  const tiering_pools_status = node_allocator.get_tiering_status(
    bucket.tiering
  );
  ...
  const bucket_last_update =
    _.get(bucket, "storage_stats.last_update") || config.NOOBAA_EPOCH;
  const system_last_update =
    _.get(bucket, "system.global.last_update") || config.NOOBAA_EPOCH;
  ...
  const info = {
    ...
    namespace: bucket.namespace
      ? {
          ...
          read_resources: _.map(
            bucket.namespace.read_resources,
            rs => pool_server.get_namespace_resource_info(rs).name
          ),
          caching: bucket.namespace.caching
        }
      : undefined,
    ...
    object_lock_configuration: config.WORM_ENABLED
      ? bucket.object_lock_configuration
      : undefined,
    tagging: bucket.tagging,
    encryption: bucket.encryption,
  ...
  _.mapKeys(_.get(bucket, "storage_stats.pools") || {}, function(
    storage,
    pool_id
  ) {
  ...
  info.triggers = _.map(bucket.lambda_triggers, trigger => {
    ...
    ret_trigger.permission_problem = !auth_server.has_bucket_permission(
      bucket,
      exec_account
    );
    return ret_trigger;
  ...
  if (bucket_stats) {
    ...
    const content_type_stats = _.get(
      bucket,
      "storage_stats.stats_by_content_type",
      []
    );
    ...
    info.stats_by_type = _.map(_.groupBy(full_stats, "data_type"), stats =>
...
function _calc_metrics({
  ...
  if (bucket.quota) {
    let quota_precent = system_utils.get_bucket_quota_usage_percent(
      bucket,
      bucket.quota
    );
  ...
  _.each(bucket.tiering.tiers, tier_and_order => {
    ...
    const tier_extra_info = tier_server.get_tier_extra_info(
      tier,
      tiering_pools_status,
      nodes_aggregate_pool,
      hosts_aggregate_pool
    ...
    info.host_tolerance = Math.min(
      info.host_tolerance || Infinity,
      tier_extra_info.host_tolerance,
      configured_failure_tolerance
    );
    info.node_tolerance = Math.min(
      info.node_tolerance || Infinity,
      tier_extra_info.node_tolerance,
      configured_failure_tolerance
    );
    info.writable = mirrors_with_valid_pool > 0;
    if (tier_and_order.tier.mirrors.length > 0) {
      if (mirrors_with_valid_pool === tier_and_order.tier.mirrors.length)
        has_enough_healthy_nodes_for_tiering = true;
      if (mirrors_with_enough_nodes === tier_and_order.tier.mirrors.length)
        has_enough_total_nodes_for_tiering = true;
    }
  });

  const used_of_pools_in_policy = size_utils.json_to_bigint(
    size_utils.reduce_sum("blocks_size", tiering_pools_used_agg)
  );
  const bucket_chunks_capacity = size_utils.json_to_bigint(
    _.get(bucket, "storage_stats.chunks_capacity") || 0
  );
  const bucket_used = size_utils.json_to_bigint(
    _.get(bucket, "storage_stats.blocks_size") || 0
  );
  const bucket_used_other = BigInteger.max(
    used_of_pools_in_policy.minus(bucket_used),
    BigInteger.zero
  );
  const bucket_free = size_utils.json_to_bigint(
    _.get(info, "tiering.storage.free") || 0
  );

  let is_storage_low = false;
  let is_no_storage = false;
  const bucket_total = bucket_free.plus(bucket_used).plus(bucket_used_other);

  const bucket_last_update =
    _.get(bucket, "storage_stats.last_update") || config.NOOBAA_EPOCH;
  const system_last_update =
    _.get(bucket, "system.global_last_update") || config.NOOBAA_EPOCH;
  const last_update = Math.max(system_last_update, bucket_last_update);
  ...
  const actual_free = size_utils.json_to_bigint(
    _.get(info, "tiering.data.free") || 0
  );
  let available_for_upload = actual_free;

  if (bucket.quota) {
    let quota_free = size_utils
      .json_to_bigint(bucket.quota.value)
      .minus(size_utils.json_to_bigint(objects_aggregate.size));
  ...
  const tier_with_issues = _.filter(
    info.tiering.tiers,
    tier => tier.mode !== "OPTIMAL"
  ).length;
...
function calc_bucket_mode(tiers, metrics, ignore_quota) {
  ...
  const issueCount =
    NO_RESOURCES + NOT_ENOUGH_RESOURCES + NOT_ENOUGH_HEALTHY_RESOURCES;

  return (
    (NO_RESOURCES === tiers.length && "NO_RESOURCES") ||
    (NOT_ENOUGH_RESOURCES === tiers.length && "NOT_ENOUGH_RESOURCES") ||
    (NOT_ENOUGH_HEALTHY_RESOURCES === tiers.length &&
    ...
    (!ignore_quota &&
      metrics.is_quota_enabled &&
      metrics.is_quota_exceeded &&
      "EXCEEDING_QUOTA") ||
...
function resolve_tiering_policy(req, policy_name) {
  var tiering_policy =
    req.system.tiering_policies_by_name[policy_name.unwrap()];
  if (!tiering_policy) {
    dbg.error("TIER POLICY NOT FOUND", policy_name);
    throw new RpcError(
      "INVALID_BUCKET_STATE",
      "Bucket tiering policy not found"
    );
  }
...
function can_delete_bucket(system, bucket) {
  return P.resolve().then(() => {
    ...
    return MDStore.instance()
      .has_any_completed_objects_in_bucket(bucket._id)
      .then(has_objects => {
        if (has_objects) {
          return "NOT_EMPTY";
        }
