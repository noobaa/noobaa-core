...
async function put_bucket_replication(req) {
    ...
    validate_replication(req, bucket.name.unwrap(), true);
...
function validate_replication(req, bucket_name, min_required) {
    ...
    if (replication_rules.length > config.BUCKET_REPLICATION_MAX_RULES ||
        (min_required && replication_rules.length < 1)) throw new RpcError('INVALID_REPLICATION_POLICY');

    let rule_ids = [];
    let pref_by_dst_bucket = {};

    _.map(replication_rules, rule => {
        ...
        if (bucket_name === destination_bucket.unwrap() || !dst_bucket) {
    ...
    if (new Set(rule_ids).size < replication_rules.length) throw new RpcError('INVALID_REPLICATION_POLICY');
...
function normalize_replication(req) {
    ...
    const validated_replication = _.map(replication_rules, rule => {
