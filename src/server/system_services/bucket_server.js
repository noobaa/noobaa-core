...
function get_bucket_changes_quota(req, bucket, quota_config, single_bucket_update, changes) {
    ...
    let quota = new Quota(quota_config);
    if (quota.get_size_value() === 0 && quota.get_quantity_value() === 0) {
        ...
        if (quota.get_size_value() < 0 || quota.get_quantity_value() < 0) throw new RpcError('BAD_REQUEST', 'quota config values must be positive');

        Quota.add_quota_alerts(system_store.data.systems[0]._id, bucket, quota, changes.alerts);
...
function _calc_metrics({
    ...
    if (bucket.quota) {
        let quota = new Quota(bucket.quota);
        info.quota = quota.get_config();

        let {size_used_percent, quantity_used_percent} = Quota.get_bucket_quota_usages_percent(bucket, quota);
        if (size_used_percent >= 100 || quantity_used_percent >= 100) {
            is_quota_exceeded = true;
        } else if (size_used_percent >= 90 || quantity_used_percent >= 90) {
            is_quota_low = true;
        }
    }
    ...
    let quota = new Quota(bucket.quota);
    let available_size_for_upload = Quota.get_available_size_for_upload(quota, actual_free, objects_aggregate.size);
    let available_quantity_for_upload = Quota.get_available_quantity_for_upload(quota, objects_aggregate.count);
