...
class Quota {
    ...
    constructor(quota_config) {
        this.size = quota_config && quota_config.size ? quota_config.size : {'value': 0};
         this.size[RAW_VALUE_FIELD_NAME] = size_utils.size_unit_to_bigint(this.size.value, this.size.unit).toJSON();
         this.quantity = quota_config && quota_config.quantity ? quota_config.quantity : {'value': 0};
         this.quantity[RAW_VALUE_FIELD_NAME] = new BigInteger(this.quantity.value).toJSON();
    }
   ...
   */
    static get_bucket_quota_usages_percent(bucket, bucket_quota) {
        var usage = {
            size_used_percent: 0,
        ...
        const quota_size = size_utils.json_to_bigint(bucket_quota.get_quota_by_size());
        if (quota_size > 0) {
            usage.size_used_percent = objects_size.multiply(100).divide(quota_size).valueOf();
        }
        const objects_count = bucket.storage_stats && size_utils.json_to_bigint(bucket.storage_stats.objects_count);
        const quantity_quota = size_utils.json_to_bigint(bucket_quota.get_quota_by_quantity());
        if (quantity_quota > 0) {
     ...
     * @returns
     */
    static add_quota_alerts(sysid, bucket, quota, collection) {
        const { size_used_percent, quantity_used_percent } = Quota.get_bucket_quota_usages_percent(bucket, quota);
        const size_alert = Quota.get_quota_alert(sysid, bucket, size_used_percent, quota.get_quota_by_size(), 'size');
        if (size_alert !== null) {
            collection.push(size_alert);
        }
        const quantity_alert = Quota.get_quota_alert(sysid, bucket, quantity_used_percent, quota.get_quota_by_quantity(), 'quantity');
     ...
     */
    static get_available_size_for_upload(quota, total_size, used) {
        let available_size_for_upload = total_size;
        if (quota.get_size_value() > 0) {
            let quota_size_free = size_utils.json_to_bigint(quota.get_quota_by_size())
     ...
     */
    static get_available_quantity_for_upload(quota, used) {
        let available_quantity_for_upload = config.QUOTA_MAX_OBJECTS;
        if (quota.get_quantity_value() > 0) {
            let quota_quantity_free = size_utils.json_to_bigint(quota.get_quota_by_quantity())
                .minus(size_utils.json_to_bigint(used));
