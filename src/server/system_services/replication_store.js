...
class ReplicationStore {
    ...
    async find_aws_log_based_replication_rules() {
        dbg.log1('find_aws_log_based_replication_rules: ');
        const replications = await this._replicationconfigs.find({ deleted: null });

        const aws_reduced_replications = _.filter(replications, repl => repl.log_replication_info.aws_log_replication_info);
        const sorted_reduced_replications = _.map(aws_reduced_replications, repl => ({
            replication_id: repl._id,
            log_replication_info: _.minBy(repl.log_replication_info, info => (info.status && info.status.last_cycle_end) || 0) //least_recently_replicated_rule
        }));
        dbg.log1('find_aws_log_based_replication_rules: ', sorted_reduced_replications);

        return sorted_reduced_replications;
    }
}
