...
mocha.describe('STS tests', function() {
    ...
    mocha.before(async function() {
        ...
        const s3accesspolicy = {
            ...
            Statement: [{
                Effect: 'Allow',
                Principal: { AWS: is_nc_coretest ? '*' : [`arn:aws:iam::${account_info_a._id.toString()}:root`, // existing gap in config_fs.is_account_exists_by_principal for NC
...
mocha.describe('Session token tests', function() {
    ...
    mocha.before(async function() {
        ...
        const s3accesspolicy = {
            ...
            Statement: [{
                Effect: 'Allow',
                Principal: { AWS: is_nc_coretest ? '*' : `arn:aws:iam::${account_info_alice._id.toString()}:root` },
                Action: ['s3:*'],
                Resource: [
