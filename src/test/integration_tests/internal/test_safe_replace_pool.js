...
mocha.describe('safe_replace_pool', function() {
    ...
    mocha.it('updates only accounts when no tiers reference old pool', async function() {
        this.timeout(90000); // eslint-disable-line no-invalid-this
        const ACCT_ONLY = `${PREFIX}-acct-only@test.com`;

        // Create account whose default_resource is OTHER_POOL (no tiers use it)
        await rpc_client.account.create_account({
            name: ACCT_ONLY,
            email: ACCT_ONLY,
            has_login: false,
            s3_access: true,
            default_resource: OTHER_POOL,
        });
        ...
        assert.strictEqual(reply.replaced_tiers, 0, 'no tiers should be replaced');
        assert(reply.updated_accounts >= 1, 'at least 1 account should be updated');
        assert.strictEqual(reply.mode, 'REPLACED');

        const account = await rpc_client.account.read_account({ email: ACCT_ONLY });
        assert.strictEqual(account.default_resource, DEFAULT_POOL_NAME);

        await rpc_client.account.delete_account({ email: ACCT_ONLY });
    });

    // Note: the migrate and finalize tests below call safe_replace_pool on
    // DEFAULT_POOL_NAME which is also attached to first.bucket by setup_pools.
    // This means first.bucket's tier and all account defaults will be switched
    // to OTHER_POOL as a side effect. Tests added after these must not assume
    // first.bucket still points at POOL_LIST[0].
