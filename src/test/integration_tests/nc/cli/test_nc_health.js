...
const os = require('os');
const { health_errors } = require('../../../../manage_nsfs/health');
...
mocha.describe('nsfs nc health', function() {
    ...
    mocha.describe('health check', function() {
        ...
        mocha.it('accounts count below limit', async function() {
            ...
            assert.strictEqual(health_status.error, undefined);
        });

        mocha.it('accounts count above limit - should emit error', async function() {
            Health.all_account_details = true;
            Health.all_bucket_details = true;
            ...
            assert.strictEqual(health_status.checks.buckets_status.count, 1);
            assert.strictEqual(health_status.checks.accounts_status.count, 1);
            assert.strictEqual(health_status.error, health_errors.ACCOUNTS_COUNT_LIMIT_WARNING);
        });
        ...
        mocha.it('buckets count above limit - should emit error', async function() {
            ...
            assert.strictEqual(health_status.checks.buckets_status.count, 1);
            assert.strictEqual(health_status.checks.accounts_status.count, 1);
            assert.strictEqual(health_status.error, health_errors.BUCKETS_COUNT_LIMIT_WARNING);
        });
