...
describe('debug_module', function() {
    ...
    it('should set level for windows style module and propogate', function() {
      var dbg = new DebugModule('C:\\Program Files\\NooBaa\\src\\agent\\agent_cli.js');
      dbg.set_level(3, 'C:\\Program Files\\NooBaa\\src\\agent');
      assert.strictEqual(dbg._cur_level.__level,3);
    });

    it('should log when level is appropriate', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        dbg.log0("test_debug_module: log0 should appear in the log");
        file_content_verify("text", "core.blabla.asd.lll:: test_debug_module: log0 should appear in the log", done);
    });

    it('should NOT log when level is lower', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        dbg.log2("test_debug_module: log2 should not appear in the log");
        file_content_verify("no_text", "test_debug_module: log2 should not appear in the log", done);
    });

    it('should log after changing level of module', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        dbg.set_level(4);
        var a = {
            out: "out",
        ...
        file_content_verify("text", "core.blabla.asd.lll:: test_debug_module: log4 should appear after level change", done);
    });

    it('should log backtrace if asked to', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        dbg.log0_withbt("test_debug_module: log0 should appear with backtrace");
        file_content_verify("text", "core.blabla.asd.lll:: test_debug_module: log0 should appear with backtrace     at", done);
    });

    it('setting a higher module should affect sub module', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        dbg.set_level(2, 'util');
        dbg.log2("test_debug_module: log2 setting a higher level module level should affect current");
        file_content_verify("text", "core.blabla.asd.lll:: test_debug_module: log2 setting a higher level module level should affect current", done);
    });

    it('formatted string should be logged correctly (string substitutions)', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        var s1 = 'this',
            s2 = 'should',
            s3 = 'expected';
        dbg.log0("%s string %s be logged as %s", s1, s2, s3);
        file_content_verify("text", "core.blabla.asd.lll:: this string should be logged as expected", done);
    });

    it('console various logs should be logged as well', function(done) {
    ...
    it('fake browser verify logging and console wrapping', function(done) {
        var dbg = new DebugModule('/web/noise/noobaa-core/src/blabla.asd/lll.asd');
        // jshint proto:true
        ...
        file_content_verify("text", "core.blabla.asd.lll:: test_debug_module: browser should appear in the log", done);
    });
});
