...
// on AWS:
// go run script/client_script.go -bucket <bucket-name> -key <key-name> -mpu <key-name-of-multi--part-upload> -endpoint "" [-disable-deletion]
// for example:
// go run script/client_script.go -bucket shira-test02 -key test-key -mpu test-mpu-key -endpoint "" -disable-deletion
// on NooBaa:
...
// go run script/client_script.go -bucket second.bucket -key test-key -mpu test-mpu-key -endpoint "https://localhost:12443" -disable-deletion
func main() {
...
func deleteObjectsAndBucket(client *s3.Client) {
	...
	enableDeletion := !disableDeletion
	if enableDeletion {
