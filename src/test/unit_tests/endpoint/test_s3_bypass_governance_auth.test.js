...
describe('s3_rest bypass governance authorization', () => {
    ...
    describe('_has_bypass_governance_permission', () => {
        ...
        it('allows NC account when bucket policy grants Bypass', async () => {
            const req = make_req({
                account: {
                ...
                // NC IAM stub Allow must not be trusted for Bypass.
                iam_result: true,
                policy: {
                    Statement: [{
                        Effect: 'Allow',
            ...
            jest.spyOn(access_policy_utils, 'get_account_identifier_id').mockReturnValue('nc-id');
            jest.spyOn(access_policy_utils, 'get_policy_principal_arn')
                .mockReturnValue('arn:aws:iam::nc-id:root');
            jest.spyOn(access_policy_utils, 'has_access_policy_permission')
                .mockResolvedValue('ALLOW');
        ...
        it('denies NC account when bucket policy does not grant Bypass (ignores IAM stub Allow)', async () => {
            const req = make_req({
                account: {
                    email: new SensitiveString('nc@example.com'),
                    name: new SensitiveString('nc-user'),
        ...
        it('denies NC account when there is no bucket policy (ignores IAM stub Allow)', async () => {
            const req = make_req({
                account: {
                    email: new SensitiveString('nc@example.com'),
                    name: new SensitiveString('nc-user'),
    ...
    describe('authorize_bypass_governance_if_requested', () => {
        ...
        it('throws AccessDenied on NC when header is set and bucket policy lacks Bypass', async () => {
            const req = make_iam_user_req({
                policy: null,
                // Hosted would treat IAM Allow as enough; NC must not.
                iam_result: true,
            });
            req.object_sdk.nsfs_config_root = '/etc/noobaa.conf.d';
            req.object_sdk.requesting_account.name = new SensitiveString('nc-user');
            // NC system_owner compare uses account name, not email.
            req._bucket_sdk_policy_info.system_owner = new SensitiveString('system-nc');
