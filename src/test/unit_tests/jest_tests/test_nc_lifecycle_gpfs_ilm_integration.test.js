...
const { file_delete } = require('../../../util/fs_utils');
...
describe('convert_expiry_rule_to_gpfs_ilm_policy unit tests', () => {
    ...
    it('convert_expiry_rule_to_gpfs_ilm_policy - expiry days', () => {
        ...
        const ilm_policy = nc_lifecycle.convert_expiry_rule_to_gpfs_ilm_policy(lifecycle_rule);
...
describe('write_tmp_ilm_policy unit tests', () => {
    ...
    nc_lifecycle.lifecycle_run_status = {
        buckets_statuses: {
            [bucket_name]: {
                rules_statuses: {
                    [lifecycle_rule_base.id]: {
                        rule_process_times: {
                            process_rule_start_time: mock_start_time,
                        }
                    }
                }
            }
    ...
    const expected_ilm_policy_path = path.join(ILM_POLICIES_TMP_DIR,
        nc_lifecycle.get_lifecycle_ilm_policy_file_name(bucket_name, lifecycle_rule_base));
    ...
    it('write_tmp_ilm_policy - ilm file does not exist', async () => {
        ...
        const ilm_policy_tmp_path = await nc_lifecycle.write_tmp_ilm_policy(bucket_name, lifecycle_rule, ilm_policy_mock_string);
...
describe('get_candidates_by_gpfs_ilm_policy unit tests', () => {
    ...
    const ilm_policy_name = nc_lifecycle.get_lifecycle_ilm_policy_file_name(bucket_name, lifecycle_rule_base);
    const ilm_policy_path = path.join(ILM_POLICIES_TMP_DIR, ilm_policy_name);
    const expected_candidates_path = path.join(ILM_CANDIDATES_TMP_DIR, 'list.' + lifecycle_rule_base.id);

    beforeEach(async () => {
        const ilm_policy_mock_string = get_mock_base_ilm_policy(bucket_path, lifecycle_rule_base.id) +
    ...
    run_or_skip_test(IS_GPFS)('get_candidates_by_gpfs_ilm_policy - ilm file does not exist - no objects returned', async () => {
        const lifecycle_rule = lifecycle_rule_base;
        const candidates_tmp_path = await nc_lifecycle.get_candidates_by_gpfs_ilm_policy(mock_bucket_json, lifecycle_rule, ilm_policy_path);
        expect(candidates_tmp_path).toBe(expected_candidates_path);
        const data = await read_file(fs_context, candidates_tmp_path);
        console.log('GPFS empty bucket path test data', data);
        expect(data).toBe('');
    });

    run_or_skip_test(IS_GPFS)('get_candidates_by_gpfs_ilm_policy - ilm file does not exist - some objects returned', async () => {
        const lifecycle_rule = { ...lifecycle_rule_base, expiration: { date: two_days_from_now } };
    ...
    run_or_skip_test(IS_GPFS)('get_candidates_by_gpfs_ilm_policy - already exists', async () => {
        const lifecycle_rule = lifecycle_rule_base;
        await create_file(fs_context, expected_candidates_path, mock_content);
        const expected_err_msg = `get_candidates_by_gpfs_ilm_policy failed with error`;
        await expect(nc_lifecycle.get_candidates_by_gpfs_ilm_policy(mock_bucket_json, lifecycle_rule,
            ilm_policy_path)).rejects.toThrow(expected_err_msg);
    });
});
...
describe('convert_lifecycle_policy_to_gpfs_ilm_policy unit tests', () => {
    ...
    it('convert_lifecycle_policy_to_gpfs_ilm_policy - expire date + filter empty', () => {
        ...
        const expected_ilm_policy = get_mock_base_ilm_policy(bucket_path, lifecycle_rule_base.id);
    ...
    it('convert_lifecycle_policy_to_gpfs_ilm_policy - expire days + filter empty', () => {
        ...
        const actual_ilm_policy = nc_lifecycle.convert_lifecycle_policy_to_gpfs_ilm_policy(lifecycle_rule, mock_bucket_json);
        const expected_ilm_policy = get_mock_base_ilm_policy(bucket_path, lifecycle_rule_base.id) +
            get_expected_ilm_expiry_days(days);
        expect(actual_ilm_policy).toBe(expected_ilm_policy);
    });
 ...
 * @returns 
 */
function get_mock_base_ilm_policy(bucket_storage_path, rule_id) {
    ...
    const policy_base = `RULE 'expList' EXTERNAL LIST ${rule_id} EXEC ''\n` +
    `RULE ${rule_id} LIST ${rule_id}\n` +
    ...
    return definitions_base + policy_base + path_policy;
}
 ...
 * @returns {String}
 */
function get_expected_ilm_expiry_days(expiry_days) {
    return `AND mod_age > ${expiry_days}\n`;
}
