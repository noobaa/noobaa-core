...
describe('manage nsfs cli bucket flow', () => {
    ...
    describe('cli create bucket', () => {
        ...
        it('should fail - cli create bucket - owner does not have any permission to path', async () => {
            ...
            const res = await exec_manage_cli(TYPES.BUCKET, action, bucket_options);
            expect(JSON.parse(res.stdout).error.message).toBe(ManageCLIError.InaccessibleBucketPath.message);
        });
    ...
    describe('cli delete bucket', () => {
        ...
        const account_defaults = {
            name: account_name,
            new_buckets_path: `${root_path}new_buckets_path_user21/`,
            uid: 999,
            gid: 999,
            access_key: 'GIGiFAnjaaE7OKD5N7hX',
            secret_key: 'G2AYaMpU3zRDcRFWmvzgQr9MoHIAsD+3oEXAMPLE',
    ...
    describe('cli update bucket', () => {
        const config_root = path.join(tmp_fs_path, 'config_root_manage_nsfs');
        const root_path = path.join(tmp_fs_path, 'root_path_manage_nsfs/');
        const bucket_name = 'bucket123';
        const bucket_storage_path = path.join(tmp_fs_path, 'root_path_manage_nsfs', bucket_name);
        const account_name = 'account-test-update';
        const account_name2 = 'account-test-update2';

        const account_defaults = {
            name: account_name,
            new_buckets_path: `${root_path}new_buckets_path_user31/`,
            uid: 1001,
            gid: 1001,
            access_key: 'GIGiFAnjaaE7OKD5N7hX',
            secret_key: 'G2AYaMpU3zRDcRFWmvzgQr9MoHIAsD+3oEXAMPLE',
        ...
        const account_defaults2 = {
            ...
            uid: 1002,
            gid: 1002,
            access_key: 'GIGiFAnjaaE7OKD5N7hY',
            secret_key: 'G2AYaMpU3zRDcRFWmvzgQr9MoHIAsD+3pEXAMPLE',
        };
