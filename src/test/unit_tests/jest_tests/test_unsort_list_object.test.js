...
describe('manage unsorted list objcts flow', () => {
    ...
    describe('Unsorted List objects ', () => {
        ...
        it('List object unsorted with multiple object upload', async () => {
            await create_keys(keys_objects);
            const ls_obj_res = await ns_tmp.list_objects({
                bucket: upload_bkt,
                delimiter: '/',
            }, dummy_object_sdk);
    ...
    describe('list objects - dirs', () => {
        ...
        it('key_marker=folder1/inner_folder/file40 delimiter=/', async function() {
            ...
            expect(r.is_truncated).toStrictEqual(false);
            const fd = folders_to_upload.filter(folder => folder > 'folder1/');
            expect(r.common_prefixes.sort()).toStrictEqual(fd.sort());
            expect(r.objects.map(it => it.key).sort()).toEqual(files_in_utf_diff_delimiter);
        });
    });
        ...
        it('page=500', async () => {
            ...
            for (;;) {
                r = await ns_tmp.list_objects({
                    bucket: upload_bkt,
                    limit: 500,
                    key_marker: r ? JSON.stringify(r.next_marker) : "",
                }, dummy_object_sdk);
                total_items += r.objects.length;
                validat_pagination(r, total_items);
                if (!r.next_marker) {
                    break;
                }
            }
...
function validat_pagination(r, total_items) {
    if (r.next_marker) {
        expect(r.is_truncated).toStrictEqual(true);
    } else {
