...
config.ENABLE_NSFS_UNSORTED_OBJECTS_LIST = true;
const tmp_nsfs_path = path.join(TMP_PATH, 'test_list_objects');
...
describe('manage unsorted list objcts flow', () => {
    ...
    describe('Unsorted List objects ', () => {
        ...
        it('List object unsorted with multiple object upload', async () => {
            await create_keys(ns_tmp, dummy_object_sdk, keys_objects);
            const ls_obj_res = await ns_tmp.list_objects({
                bucket: upload_bkt,
                delimiter: '/',
            }, dummy_object_sdk);
    ...
    describe('list objects - dirs', () => {
        ...
        beforeAll(async () => {
            config.ENABLE_NSFS_UNSORTED_OBJECTS_LIST = true;
            await fs_utils.create_fresh_path(ns_tmp_bucket_path);
            await create_keys(ns_tmp, dummy_object_sdk, [
                ...folders_to_upload,
        ...
        afterAll(async function() {
            config.ENABLE_NSFS_UNSORTED_OBJECTS_LIST = false;
            await delete_keys(ns_tmp, dummy_object_sdk, [
                ...folders_to_upload,
                ...files_in_folders_to_upload,
                ...files_without_folders_to_upload,
                ...files_in_utf_diff_delimiter,
        ...
        it('key_marker=folder229/', async () => {
            ...
            expect(r.objects.map(it => it.key)).toStrictEqual([...fd, ...files_in_utf_diff_delimiter]);
        });
        ...
        it('key_marker=folder229', async function() {
            ...
            expect(r.common_prefixes).toStrictEqual([]);
            const fd = folders_to_upload.filter(folder => folder >= 'folder229/');
            expect(r.objects.map(it => it.key)).toEqual([...fd, ...files_in_utf_diff_delimiter]);
        });
        ...
        it('key_marker=folder1/', async function() {
            ...
            expect(r.objects.map(it => it.key).sort()).toEqual([...files_in_inner_folders_to_upload_pre,
        ...
        it('key_marker=folder1/file57', async function() {
            ...
            expect(r.objects.map(it => it.key)).toEqual([...fd1, ...files_in_inner_folders_to_upload_post,
                ...fd, ...files_in_utf_diff_delimiter]);
        });
        it('key_marker=folder1/inner_folder/file40', async function() {
            ...
            const fd = folders_to_upload.filter(folder => folder > 'folder1/');
            expect(r.objects.map(it => it.key)).toEqual([...fd1, ...fd, ...files_in_utf_diff_delimiter]);
        });
        ...
        it('key_marker=folder1/ainner_folder/', async function() {
            ...
            expect(r.objects.map(it => it.key).sort()).toEqual([...fd2, ...files_in_folders_to_upload,
                ...files_in_inner_folders_to_upload_post,
                ...fd, ...files_in_utf_diff_delimiter
            ].slice(0, 1000).sort());
        });

        it('key_marker=folder1/inner_folder/file40 delimiter=/', async function() {
            const r = await ns_tmp.list_objects({
                bucket: upload_bkt,
                key_marker: 'folder1/inner_folder/file40',
                delimiter: '/'
            ...
            expect(r.is_truncated).toStrictEqual(false);
            const fd = folders_to_upload.filter(folder => folder > 'folder1/');
            expect(r.common_prefixes).toStrictEqual(fd);
            expect(r.objects.map(it => it.key)).toEqual(files_in_utf_diff_delimiter);
        });
    });
});
...
async function delete_keys(ns_tmp, dummy_object_sdk, keys) {
...
async function create_keys(ns_tmp, dummy_object_sdk, keys) {
