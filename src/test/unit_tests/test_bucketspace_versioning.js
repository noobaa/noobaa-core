    ...
    mocha.describe('versioning enabled', function() {
        ...
        mocha.describe('put object', function() {
            ...
            mocha.it('put object - versioning enabled - nested key (more than 1 level)', async function() {
                ...
                await s3_uid6.putObject({ Bucket: nested_keys_bucket_name, Key: nested_key_level3, Body: body1 });
                // only after the second PUT object we expect to have the .versions under the parent directory of the file
                await s3_uid6.putObject({ Bucket: nested_keys_bucket_name, Key: nested_key_level3, Body: body2});
                await s3_uid6.putObject({ Bucket: nested_keys_bucket_name, Key: nested_key_level3, Body: body3});
