...
coretest.describe_mapper_test_case({
    ...
    const frags = _.concat(
        _.times(data_frags, data_index => ({ _id: (new ObjectID(null)).toString(), data_index })),
        _.times(parity_frags, parity_index => ({ _id: (new ObjectID(null)).toString(), parity_index }))
    );
    const first_pools = _.times(num_pools, i => ({ _id: (new ObjectID(null)).toString(), name: 'first_pool' + i, }));
    const second_pools = _.times(num_pools, i => ({ _id: (new ObjectID(null)).toString(), name: 'second_pool' + i, }));
    const external_pools = _.times(num_pools, i => ({ _id: (new ObjectID(null)).toString(), name: 'external_pool' + i, }));
    ...
    function make_block(params = {}, i = 0) {
        ...
        const _id = (new ObjectID(null)).toString();
