...
mocha.describe('new tests check', async function() {
    ...
    mocha.it('NON ROOT 3 without the files gid inside the suplemental group - failure', async function() {
        await nb_native().fs.mkdir(NON_ROOT2_FS_CONFIG, full_path_non_root2, 0o770);
        try {
            //non root3 doesn't have non-root1 group as supplemental group, so it should fail
