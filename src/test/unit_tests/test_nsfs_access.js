...
mocha.describe('new tests check', async function() {
    ...
    mocha.before(async function() {
        ...
        await nb_native().fs.mkdir(NON_ROOT1_FS_CONFIG, full_path_non_root, 0o770);
    ...
    mocha.it('NON ROOT 1 mkdir - failure', async function() {
        const new_folder = "folder1";
        const new_path = path.join(full_path_root, new_folder);
        try {
            const mkdir_res = await nb_native().fs.mkdir(NON_ROOT1_FS_CONFIG, new_path, 0o770);
    ...
    mocha.it('ROOT readdir - dir created by non dir - success', async function() {
            await nb_native().fs.readdir(ROOT_FS_CONFIG, full_path_non_root);
    });
    ...
    mocha.it('NON ROOT with suplemental group - success', async function() {
        const non_root_entries = await nb_native().fs.readdir(NON_ROOT3_FS_CONFIG, full_path_non_root);
        assert.equal(non_root_entries && non_root_entries.length, 0);
    });

    mocha.it('NON ROOT with different suplemental group - failure', async function() {
        try {
