...
async function get_next_object(prefix) {
    while (_list_objects_next >= _list_objects.Contents.length && _list_objects.IsTruncated) {
        if (_list_objects_promise) {
            ...
            if (!_list_objects.IsTruncated) {
                console.log('get_next_object: total objects count', _list_objects.Contents.length);
            }
        }
    }

    if (_list_objects_next >= _list_objects.Contents.length) {
