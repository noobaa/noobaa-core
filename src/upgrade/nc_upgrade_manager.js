 ...
 */
async function update_rpm_upgrade(config_fs) {
...
async function upgrade_config_dir(config_fs, upgrade_options = {}) {
    let system_data = await config_fs.get_system_config_file({ silent_if_missing: true });
    if (!system_data) throw new Error('system does not exist');

    if (!system_data.config_directory) system_data.config_directory = config_directory_defaults(system_data);
    const config_dir_from_version = system_data.config_directory.config_dir_version;
    const config_dir_to_version = config_fs.config_dir_version;
 ...
 * @returns {Promise<Object>}
*/
async function _update_config_dir_upgrade_start(config_fs, system_data, options) {
 ...
 * @returns {Promise<Void>}
 */
async function _update_config_dir_upgrade_finish(config_fs, system_data, this_upgrade) {
    ...
    const updated_config_directory = {
        ...
        upgrade_package_version: this_upgrade.to_version,
        upgrade_history: {
            ...upgrade_history,
