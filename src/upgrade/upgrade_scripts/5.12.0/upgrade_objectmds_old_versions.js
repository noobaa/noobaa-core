...
// 1. update old versions of an object with version_past = true
// 2. drop the current index - idx_btree_objectmds_latest_version_index

// idx_btree_objectmds_latest_version_index postgres index is incorrect since 
...
async function run({ dbg, db_client }) {
    ...
    try {
        if (process.env.DB_TYPE !== 'postgres') {
            dbg.log0('upgrade postgres objectmds latest index: not running on postgres - no upgrade needed...');
            return;
        }
        dbg.log0('starting upgrade version past on non latest object versions...');
        ...
        const batch_handler = async rows => {
            ...
            // update in db the non latest version ids with version_past = true
            let update_q = `UPDATE objectmds 
                SET data = jsonb_set(data, '{version_past}','true'::jsonb) 
        ...
        dbg.log0('upgrade_objectmds: finished updating all non latest versions with version_past=true');

        // 2. drop index
        const index_short_name = `latest_version_index;`;
        ...
        dbg.error('upgrade_objectmds:got error while upgrading buckets version past:', err);
        throw err;
    } finally {
...
module.exports = {
    run,
    description: 'Update objectmds version_past for non latest version'
};
