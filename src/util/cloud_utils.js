...
function _create_azure_connection(conn_str, params) {
    ...
    try {
        ...
        dbg.warn(`got error on BlobServiceClient.fromConnectionString with params`, _.omit(params, 'secret', 'secret_key'), ` error: ${err}`);
        throw err;
    }
    return blob;
}
...
function _create_azure_sts_connection(params) {
    ...
    try {
        ...
        dbg.error(`got error on BlobServiceClient() with params`, _.omit(params.azure_sts_credentials, 'azure_tenant_id'), ` error: ${err}`);
        throw err;
    }
    return blob;
}
