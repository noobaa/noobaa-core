...
class FileReader extends stream.Readable {
    ...
    constructor({ fs_context,
        ...
        highWaterMark = config.NFSF_DOWNLOAD_STREAM_MEM_THRESHOLD,
    }) {
        super({ highWaterMark });
        ...
        this.stat = stat;
        this.start = start || 0;
        this.end = Math.min(stat.size, end ?? Infinity);
        this.pos = this.start;
    ...
    async read_into_buffer(buf, offset, length) {
        ...
        let nread = 0;
        if (process.env.GGG_SKIP_IO === 'true' || process.env.GGG_SKIP_IO_READ === 'true') {
            const nnn = (await this.file.read(this.fs_context, buf, offset + length - 1, 1, this.pos + length - 1));
            nread = nnn ? nnn + length - 1 : 0;
        } else {
            nread = await this.file.read(this.fs_context, buf, offset, length, this.pos);
        }
        if (nread) {
            this.pos += nread;
