...
class FrameStream {
    ...
    _send_message(buffers, message_type_code_16bit) {
        const msg_len = _.sumBy(buffers, 'length');
    ...
    _on_data(data) {
        ...
        while (run) {
            ...
            const msg_len = this._msg_header.readUInt32BE(this._magic_len + 4);
            ...
            const msg = buffer_utils.extract(this._buffers, msg_len);
            const msg_type = this._msg_header.readUInt16BE(this._magic_len + 2);
