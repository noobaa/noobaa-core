...
class Speedometer {
    ...
    reset() {
        ...
        this.min_latency = -1;
        this.max_latency = -1;
        this.latency_sketch = new sketches.DDSketch({ relativeAccuracy: 0.001 });
    ...
    async _run_workers() {
        if (cluster.isWorker) {
            ...
            /** @type {Message} */
            const msg = { op: 'run' };
            for (const w of Object.values(this.workers)) w.send(msg);
        } else {
            // run as primary
            this.reset();
